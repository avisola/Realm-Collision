...
public class ActionPanel extends JPanel {
    ...
    public void setActionInfo(String info) {
        actionInfoLabel.setText(info);
    }

    public void flashButton(String actionCommand) {
        JButton buttonToFlash = null;
        switch (actionCommand) {
            case "ATTACK": buttonToFlash = attackBtn; break;
            case "DEFEND": buttonToFlash = defendBtn; break;
            case "SKILL": buttonToFlash = skillBtn; break;
            case "ULTIMATE": buttonToFlash = ultimateBtn; break;
            case "SECOND_ULTIMATE": buttonToFlash = secondUltimateBtn; break;
        }

        if (buttonToFlash != null) {
            Color originalColor = buttonToFlash.getBackground();
            Timer flashTimer = new Timer(200, null);
            JButton finalButtonToFlash = buttonToFlash;
            flashTimer.addActionListener(e -> {
                if (finalButtonToFlash.getBackground().equals(originalColor)) {
                    finalButtonToFlash.setBackground(brighter(originalColor));
                } else {
                    finalButtonToFlash.setBackground(originalColor);
                    flashTimer.stop();
                }
            });
            flashTimer.start();
