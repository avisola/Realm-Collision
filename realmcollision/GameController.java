...
public class GameController implements ActionListener {
    ...
    private void initializeGame(String p1Code, String p2Code) {
        // Create characters based on selection using CharacterStats
        ...
        // Connect battle log to characters
    ...
    public void handleAction(String action) {
        ...
        // Handle post-action regeneration
        ...
        if (damage > 0) {
            ...
            // Special handling for Wanderer with clones
        ...
        // Check for extra turns (Trickster)
        if (currentPlayer instanceof Trickster) {
            ...
            if (trickster.hasExtra()) {
                ...
                // Force immediate UI update to show health bar changes
                gui.forceImmediateUpdate();

                gui.getBattleLog().addMessage("Trickster gets an EXTRA TURN!");
                trickster.useExtra();

                // Update display again after using extra turn
                gui.forceImmediateUpdate();
    ...
    private void updateDisplay() {
        ...
        // Show/hide second ultimate button for Wanderer
        realmcollision.classes.Character currentPlayer = (currentTurn == 1) ? player1 : player2;
        gui.getActionPanel().showSecondUltimate(currentPlayer instanceof Wanderer);

        // Update special status for burn effects - use getStatus() method instead of direct class check
